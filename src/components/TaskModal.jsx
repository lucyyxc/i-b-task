  ...
  const closeModal = (save) => {
    if (save && !state.newTask) {
      ...
      if (state.changes.status)
        updates.push(
          axios.post('/api/post/statusUpdate', {
            id: state.task.id,
            status: state.changes.status,
          })
        );
      if (state.changes.startdate)
        updates.push(
          axios.post('/api/post/startDateUpdate', {
            id: state.task.id,
            startdate: state.changes.startdate,
          })
        );
      if (state.changes.enddate)
        updates.push(
          axios.post('/api/post/endDateUpdate', {
            id: state.task.id,
            enddate: state.changes.enddate,
          })
        );
      if (state.changes.notes)
        updates.push(
          axios.post('/api/post/notesUpdate', {
            id: state.task.id,
            notes: state.changes.notes,
          })
        );
      if (state.changes.assignee)
        updates.push(
          axios.post('/api/post/assigneeUpdate', {
            id: state.task.id,
      ...
      if (state.changes.archive)
        updates.push(
          axios.post('/api/post/archiveTask', { id: state.task.id })
        );
      if (state.changes.tasklabel)
        updates.push(
          axios.post('/api/post/nameUpdate', {
            id: state.task.id,
            tasklabel: state.changes.tasklabel,
          })
        );
      ...
      const newTaskBody = {
        ...
        startdate:
          state.changes.startdate || moment(new Date()).format('YYYY-MM-DD'),
        enddate:
          state.changes.enddate || moment(new Date()).format('YYYY-MM-DD'),
        notes: state.changes.notes,
      ...
      axios
        .post('/api/post/createTask', newTaskBody)
        .then((response) => {
          setState({
            ...state,
            task: {},
      ...
      <div className='modal-container'>
        ...
        <div className='modal-content'>
          <div className='status-box'>
              ...
              <DatePicker
                ...
                selected={
                  state.task.startdate
                    ? moment(state.task.startdate).toDate()
                    : new Date()
                }
                ...
                onChange={(date) => {
                  if (moment(state.task.enddate).isBefore(moment(date))) {
                    toast.error(
                      'You cannot select a Complete by date that is before the Start on date.',
                      {
                        position: 'top-center',
                        autoClose: 4000,
            ...
            <div className='status-holder'>
              ...
              <DatePicker
                ...
                selected={
                  state.task.enddate
                    ? moment(state.task.enddate).toDate()
          ...
          <div className='advice'>
            <span className='title'>Advice from The Independent Bride</span>
            <span className='the-advice'>
              {_get(
                state.task,
                'advice',
                'Your task your rules! Add any notes or links to the section below.'
              )}
            </span>
          </div>
          <div className='notes'>
