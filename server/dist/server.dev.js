...
app.post('/api/post/newUser', function (req, res) {
  ...
  db.create_user([body.name, body.email, body.user_metadata.emailAgree, initials, body.user_metadata.weddingdate, body.user_metadata.birthday, false, null, false, body.user_metadata.timeUntil, body._id]).then(function (user) {
    ...
    var newTasks = [user[0].auth_id, user[0].assignee].concat(taskStarts.map(function (e) {
      return dateFromWedding(Math.round(e * taskTimeMultiplier));
    })).concat(taskEnds.map(function (e) {
      return dateFromWedding(Math.round(e * taskTimeMultiplier));
    }));
...
var taskStarts = [365, 364, 363, 362, 361, 361, 360, 359, 358, 360, 357, 333, 242, 328, 350];
var taskEnds = [362, 357, 359, 338, 361, 358, 337, 352, 358, 327, 305, 331, 210, 321, 326];